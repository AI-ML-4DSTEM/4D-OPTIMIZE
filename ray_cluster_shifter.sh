...
train="unrot_augmented_train.tfrecords"
test="unrot_augmented_test.tfrecords"
...
shifter --env PYTHONUSERBASE=${PYTHONUSERBASE} \
        ...
        bash -c 'python -u tune_training.py --train-dataset "$train" --test-dataset "$test" --num-ray-hosts ${#nodes[@]} --num-ray-workers-per-host ${SLURM_GPUS_PER_TASK} --num-ray-cpus-per-worker "$RAY_NUM_CPUS_PER_WORKER" --num-ray-gpus-per-worker "$RAY_NUM_GPUS_PER_WORKER"'
end_time="$(date -u +%s)"

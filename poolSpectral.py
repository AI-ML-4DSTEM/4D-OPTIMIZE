...
class SpectralMaxPool2D(Layer):
  ...
  def __init__(self, img_size, pool_size = (2,2), strides = (1,1), alpha = 0.3, beta = 0.15 , gamma = 0.50, kappa = 0.50,
  ...
  def call(self, inputs):
    ...
    im_fft = rfft2d(inputs_fft)
    
    filter_size = self._get_sp_dim(self.img_size)
    #filter_size = math.multiply(self.gamma, img_size)
    ...
    #outputs, _ = normalize(outputs, axis = (1,2))  
...
class ComplexSpectralMaxPool2D(Layer):
  ...
  def __init__(self, pool_size = (2,2), strides = (1,1), alpha = 0.3, beta = 0.15 , gamma = 0.50, kappa = 0.50,
               padding='valid', data_format=None,
               name=None, **kwargs):
  ...
  def call(self, inputs):
    ...
    im_fft = dtypes.complex(inputs[:,:,:,:self.input_channel//2], inputs[:,:,:,(self.input_channel//2):])

...
class image_augmentation(object):
    ...
    @tf.function
    def augment_img(self, inputs: utils.TensorLike, probe = None):
    ...
    @tf.function
    def scale_image(self, inputs: utils.TensorLike):
        '''
    ...
    @tf.function
    def shot_noise(self, inputs: utils.TensorLike):
        #input_shape = inputs.shape
    ...
    @tf.function
    def backgrnd_plasmon(self, inputs: utils.TensorLike, probe: utils.TensorLike):
        """
        ...
        CBEDout = inputs * (1-self.weightBackgrnd) + tf.cast(CBEDbgConv, tf.float32) * self.weightBackgrnd
